...
export default function AdminPanel() {
  ...
  const actualizarEstadoCita = async (id: string, estado: string) => {
    try {
      ...
      window.alert('No se pudo actualizar la cita');
    }
  };
  ...
  const eliminarCitaAdmin = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        // No filtramos — actualizamos estado localmente para que se vea como cancelada
  ...
  const renderCitas = () => (
    loadingCitas ? (
      ...
      <View style={s.emptyBox}>
        <Text style={s.emptyText}>{'No hay citas'}</Text>
      </View>
    ) : (
      citas.map((cita) => (
        <View key={cita.id} style={[s.itemCard, { backgroundColor: estadoCitaColor(cita.estado) }]}>
          ...
          {cita.perfiles && (
            <View style={s.clienteBox}>
              ...
              <Text style={s.clienteNombre}>
                {cita.perfiles.nombre}{' '}{cita.perfiles.apellidos}
              </Text>
            </View>
          )}
          ...
          {cita.estado === 'cancelada' && cita.cancelador && (
            <View style={s.canceladoBox}>
              ...
              <Text style={s.canceladoNombre}>
                {cita.cancelador.nombre}{' '}{cita.cancelador.apellidos}
              </Text>
            </View>
          )}

          {cita.aclaracion ? (
            <Text style={s.itemNota}>{'Nota: '}{cita.aclaracion}</Text>
          ) : null}
          ...
          <View style={s.accionesRow}>
            {['pendiente','confirmada','completada','cancelada'].map((est) => (
              ...
              >
                <Text style={[s.estadoBtnText, cita.estado === est && s.estadoBtnTextActive]}>
                  {est}
                </Text>
              </TouchableOpacity>
            ))}
            <TouchableOpacity
              style={s.eliminarBtn}
              onPress={() => eliminarCitaAdmin(cita.id)}
            >
  ...
  const renderReservas = () => (
    loadingReservas ? (
      ...
      <View style={s.emptyBox}>
        <Text style={s.emptyText}>{'No hay reservas de productos'}</Text>
      </View>
    ) : (
        ...
        <View key={reserva.id} style={[s.itemCard, { backgroundColor: estadoReservaColor(reserva.estado) }]}>
          ...
          {reserva.perfiles && (
            <View style={s.clienteBox}>
              <Text style={s.clienteLabel}>{'Cliente:'}</Text>
              <Text style={s.clienteNombre}>
                {reserva.perfiles.nombre}{' '}{reserva.perfiles.apellidos}
              </Text>
            </View>
          )}

          {reserva.nota ? (
            <Text style={s.itemNota}>{'Nota: '}{reserva.nota}</Text>
          ) : null}
          ...
          <View style={s.accionesRow}>
            {['pendiente','lista','recogida','cancelada'].map((est) => (
              ...
              >
                <Text style={[s.estadoBtnText, reserva.estado === est && s.estadoBtnTextActive]}>
                  {est}
                </Text>
              </TouchableOpacity>
            ))}
      ...
      productos.map((producto) => (
        <View key={producto.id} style={s.itemCard}>
          ...
          {producto.descripcion ? (
            <Text style={s.itemNota}>{producto.descripcion}</Text>
          ) : null}
          <View style={s.accionesRow}>
            <TouchableOpacity
              style={s.eliminarBtn}
...
const s = StyleSheet.create({
  ...
  clienteBox: {
    ...
    alignItems: 'center',
    gap: 6,
    backgroundColor: 'rgba(99,32,44,0.06)',
    paddingHorizontal: 8,
    paddingVertical: 4,
    borderRadius: 4,
  },
